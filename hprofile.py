...
def instance_jaccard(profile1, profile2, neuron=False):
    ...
    if neuron:
        aprofile = set(
            [(layer, neuron) for layer in profile1.neuron_counts for neuron in profile1.neuron_counts[layer]])
        bprofile = set(
            [(layer, neuron) for layer in profile2.neuron_counts for neuron in profile2.neuron_counts[layer]])
    else:
        aprofile = set(
            [(layer, synapse) for layer in profile1.synapse_counts for synapse in profile1.synapse_counts[layer]])
        bprofile = set(
            [(layer, synapse) for layer in profile2.synapse_counts for synapse in profile2.synapse_counts[layer]])
...
def order_neuron_counts(profile):
    ...
    for layer in profile.neuron_counts.keys():
        count_dict[layer] = sorted([(k, v) for k, v in profile.neuron_counts[layer].items()], key=lambda x: x[1],
                                   reverse=True)
    return count_dict
